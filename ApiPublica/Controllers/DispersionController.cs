...
namespace ApiPublica.Controllers
{
    ...
    public class DispersionController : BaseController
    {
        ...
        [HttpGet(RoutesPath.DispersionController_ListPayOut)]
        [TypeFilter(typeof(AllowAnonymousFilter))]
        [ResponseCache(Duration = 15, Location = ResponseCacheLocation.Client)]
        public async Task<ActionResult> ListPayOut()
        {
            BaseResponse response = new BaseResponse();
            try
            {
                ValidateAccess(RoutesPath.DispersionController_ListPayOut, new { });
